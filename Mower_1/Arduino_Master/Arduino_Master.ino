/* This sketch contains the code for the 'Master' Arduino; implementing the Wifi, CAN bus communication and SD card logging.
*/
#include "Wire_Jens.h"
#include "WiFiEsp.h"
#include <SPI.h>
...
/*
   Global variables holding the most recent data from:
   - Sensors
   - Temperature sensor of the ventilator
*/
...
/*
   Variables used for timing; When x second have past without any new data received -> Do an update.
*/
uint32_t time_start;
uint32_t time_now;

/*
   Booleans for indicating wether there was a new CAN bus message.
   An update needs to be requested from the slave.
   Or the SD card should be synched with the Android phone.
*/
bool can_update;
...
/*
   Flagrecv is set in the interrupt routine of the CAN bus buffer.
   ...
   The switch statement is used to distinguish between the different sensors.
   Each sensor has a distinct CAN ID.
   CAN ID = 10, is the ventilator module. It will can only send 3 values: 57,58,59.
   Each value stands for a mode it is operating is: Clean, Cooling, Idle.
   A special case is when there is a motor fault.
   */
void CanGetData()
{
  if (flagRecv)
...
/*
   Sends data to the Android phone.
   Return the amount of characters send to the Android phone.
*/
...
/*
   Log received data on the SD card.
   Open the File in Write mode and print data to a line.
*/
...
/*
   This function will sync any data saved on the SD card with the Android phone.
   It will read the File line by line and send it over wifi.
   When every line has been written, the file will be deleted.
*/
